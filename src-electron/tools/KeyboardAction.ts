...
export class KeyboardAction implements KeyboardProviderInterface {
  ...
  public static keyLookup(key: Key) {
    return this.KeyLookupMap.get(key);
  }

  private static mapModifierKeys(...keys: Key[]) {
    return keys
  ...
  public async pressKey(...keys: Key[]) {
    keys.reverse();
    const [key, ...modifiers] = keys;
    if (key) {
  ...
  public async releaseKey(...keys: Key[]) {
    keys.reverse();
    const [key, ...modifiers] = keys;
    if (key) {
